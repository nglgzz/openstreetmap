# This file contains the code used on the quizzes in lesson 6
...
def shape_element(element):
    ...
    if element.tag == "node" or element.tag == "way" :
        ...
        for key in attrs.keys():
            if key not in CREATED:
                if key != "lon" and key!= "lat":
                    ...
                    if key == "lon":
                        pos[1] = float(attrs[key])
                    else:
                        pos[0] = float(attrs[key])
            else:
        ...
        for tag in element:
            if tag.tag == "tag":
                attrs = tag.attrib
                if not problemchars.search(attrs["k"]) and attrs["k"].count(":") <= 1:
                    if attrs["k"][0:5] == "addr:":
                        addr = attrs["k"].split(":")
                        address[addr[1]] = attrs["v"]
                        print attrs["k"]
        if pos != [None, None]:
            node["pos"] = pos
